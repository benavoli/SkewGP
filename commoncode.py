    ...
    while mc < nsamples+tune:
        ...
        for i in range(thetas_1.shape[0]):
            if np.isnan(thetas_1[i,0]+thetas_2[i,0])==0:
                ...
                if x0[i]*np.cos(thetas_1[i]+eps) +nu[i]*np.sin(thetas_1[i]+eps)>=trunc[i]/np.sqrt(eigen_max):
                    vmin = thetas_1[i]
                    vmax = thetas_2[i]
                elif x0[i]*np.cos(thetas_1[i]-eps) +nu[i]*np.sin(thetas_1[i]-eps)>=trunc[i]/np.sqrt(eigen_max):
                    vmin = thetas_2[i]
                    vmax = thetas_1[i]
        ...
        if empty == False:
            ...
            mc=mc+1
            if progress==True:
                printProgressBar(mc, nsamples+tune, prefix = 'Progress:', suffix = 'Complete', length = 50)
        else:
