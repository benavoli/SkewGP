function Ret=main_BO_alessio(X,y,ell,sigmae2,hyper_opt)
...
% by Chu, Ghahramani, ICML, 2005
        ...
        opts.M = size(y,1);   % Number of pairwise comparisons for training
        opts.N = size(X,1);    % Number of experiments for training
        %Mtest = size(ytest,1);    % Number of pairwise comparisons for testing
        %Ntest = size(Xtest,1);     % Number of experiments for testing
        
        opts.hyper_opt = hyper_opt; % set opts.hyper_opt = 1 if kernel hyper-parameters should be selected by maximizing the marginal likelihood 

        % Initial values of kernel hyper-parameters
        opts.SE.l2 = ell.^2; %0.2*ones(size(X,2),1);   %length-scale square
        opts.SE.sigmaf2 = sigmae2;   %kernel variance
        opts.sigmae2 =  1.0/2;  %1.0; % noise variance
...
%         end
        ...
        %[m_vec, s_vec, s2_mat] = GP_mu_s(Xtest,D,opts); %compute mean and standard deviation of the esimated GP on the test data
        
        Ret = {opts.SE, opts.sigmae2, D.fMAP, opts.Sigma, opts.LambdaMAP, D.log_pD};
